...
const SystemWord = () => {
		...
		try {
			...
			imgs.forEach(img => {
				...
				if (fileId) {
					...
					if (!nextSrc || isBrokenFileSrcValue(nextSrc)) {
						nextSrc = `files/${fileId}`
						img.setAttribute('src', nextSrc)
					}
				}

				// data/blob/Base64 srclarni saqlamaymiz; faqat tozalaymiz
				if (nextSrc.startsWith('data:') || nextSrc.startsWith('blob:')) {
					img.removeAttribute('src')
				} else if (!fileId && isBrokenFileSrcValue(nextSrc)) {
					img.removeAttribute('src')
				}
	...
	const sanitizeLiveImageElement = img => {
		...
		if (!fileId) {
			img.removeAttribute('src')
		} else if (src.startsWith('data:') || isBrokenFileSrcValue(src)) {
			img.removeAttribute('src')
		}
		img.removeAttribute('data-src-resolved')
		img.removeAttribute('data-upload-progress')
	...
	const saveAllChanges = async () => {
		...
		try {
			...
			allDocImages.forEach(img => {
				...
				// blob previewlarni saqlamaslik uchun src ni tozalaymiz
	...
	const handlePasteImage = async (file, imgElement) => {
		try {
			...
			// Local preview with object URL (persisted HTML won't store it)
			if (imgElement) {
				try {
					const objectUrl = URL.createObjectURL(uploadFile)
					imgElement.src = objectUrl
					imgElement.dataset.tempObjectUrl = objectUrl
				} catch (e) {
					imageLogError('handlePasteImage preview error', e)
				}
			}
			...
			if (fileId && imgElement) {
				...
				// preview src object URL bo'lib qoladi; saqlash paytida tozalanadi
			}
	...
	useEffect(() => {
		...
		const run = async () => {
			...
			for (const img of imgs) {
				...
				// Resolved deb faqat blob URL ni hisoblaymiz; files/... yoki bo'sh src bo'lsa ham yuklaymiz.
				const hasResolvedSrc = currentSrc.startsWith('blob:')
				const skipResolved =
